...
function post_build_image__900_convert_to_abl_img() {
	...
	if [[ "${abl_boot_partition_label}" =~ ^boot_ ]]; then
		 declare -g bootimg_cmdline="${BOOTIMG_CMDLINE_EXTRA} root=${rootdev} slot_suffix=${abl_boot_partition_label#boot} ${extraargs}"
	else
		 declare -g bootimg_cmdline="${BOOTIMG_CMDLINE_EXTRA} root=${rootdev} ${extraargs}"
	fi
